...
public class RentalController {
    ...
    @GetMapping("/itemList")
    @Operation(summary = "물품 조회", description = "대여가능한 전체 물품 목록을 조회합니다.\n카테고리 타입에 따라 조회할 수 있습니다.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "404", description = "실패"),
    })
    public ResponseEntity<Void> getItemList() {
    ...
    public ResponseEntity<Void> postRental() {
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<Void> getUser(@Parameter(description = "사용자 ID", required = true, example = "1")
